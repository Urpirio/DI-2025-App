    ...
    fetch(GlobalApis.ApiUserInEvent + LocalData.IDEvents, {
      ...
      .then((Data) => {
        ...
        TodosParticipantes.forEach((D) => {
          if (
            TodosID.filter((t) => t.user_id == D.id && t.checkin != null)
              .length > 0
          ) {
            D.Registrado = true;
        ...
        if (
          TodoGetTodosUsuarios.length > 0 &&
          TextSearch == "" &&
          FiltroEstado == "Todos los usuarios"
        ) {
          ...
          if (FiltroBuscador === "Email") {
            TodoGetTodosUsuarios.forEach((D) => {
              ...
              if (
                NombreCompleto.toUpperCase().includes(TextSearch.toUpperCase())
              ) {
