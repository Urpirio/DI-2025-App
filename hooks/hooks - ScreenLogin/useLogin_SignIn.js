...
export const useLogin_SignIn = () =>{
    ...
    const ValidarInformacion = async ({CredentialShow,camposVacios,correoInvalido}) =>{
    ...
    const SignIn = async ({CredentialShow,camposVacios,correoInvalido,Password})=>{ 
    ...
    if(!TextEmail.includes('@') && (TextEmail != '' && TextPassword != '')){
      ...
      //condicional de todo correcto
        ...
        if(Password){
          PostUserCredential({Email:TextEmail,Password:Password,ErrorFunction: CredentialShow});
        }else{
